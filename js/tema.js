/* Pinta tema.html a partir del parametro ?id=.
   1. Busca en data/curso.json a que unidad pertenece esa sesion.
   2. Carga el contenido completo desde data/unidades/<unidad>/<id>.json. */
...
async function cargarTema() {
  ...
  if (!id) {
         raiz.innerHTML = '<p class="vacio">No se ha especificado ninguna sesion.</p>';
         return;
  }
  ...
  try {
       ...
       if (!unidadDeLaSesion) {
                raiz.innerHTML = '<p class="vacio">No se ha encontrado esta sesion.</p>';
                return;
       }
       ...
       document.title = `${tema.titulo} - Operaciones Basicas`;
       ...
       pintarLeccion(tema.contenido);
         ...
         raiz.innerHTML = '<p class="vacio">No se ha podido cargar la sesion. Si estas probando el sitio en tu ordenador, recuerda abrirlo con un servidor local (ver README).</p>';
         console.error(error);
  }
}
